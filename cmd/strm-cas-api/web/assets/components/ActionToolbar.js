  ...
  template: `
    <div class="section card">
      ...
      <div class="toolbar section">
        ...
        <label class="muted">单文件大小上限(GB, 0=不限) <input type="number" min="0" step="1" :value="settings.max_file_size_gb" @input="$emit('update-settings', { key: 'max_file_size_gb', value: Number($event.target.value || 0) })" style="width:100px" /></label>
      ...
      <div class="toolbar section">
        ...
        <button @click="$emit('toggle-auto-refresh')" class="secondary">WS自动重连：{{ autoRefreshEnabled ? '开' : '关' }}</button>
        ...
        <span class="muted">单文件上限: {{ settings.max_file_size_gb > 0 ? settings.max_file_size_gb + ' GB' : '不限' }}</span>
      </div>
